El ejercicio 1 es un formulario que debe solicitar tres datos:
Nombre de la persona, Edad y Cantidad de tarros de pintura.

Cada tarro cuesta $9000.

Según la edad, se aplica el siguiente descuento:
Menores de 18 años: sin descuento, Entre 18 y 30 años (incluidos): 15% de descuento y Mayores de 30 años: 25% de descuento

Al enviar el formulario, la página debe mostrar:
Nombre del cliente, Total sin descuento (cantidad de tarros × 9000) y
Total a pagar

En caso que el cliente cumpla con los requistos de descuento, se aplicara
el descuento correspondiente según la edad

--------------------------------------------------------------------

El ejercicio 2 es una pagina de inicio de sesion que tiene 2 usuarios registrados,
un usuario es administrador y el otro es usuario comun.
al ingresar sus datos el programa validara la informacion, si los datos ingresados son correctos, se desplegara un mensaje de bienvenida,
caso contrario arrojara un mensaje se "usuario o contraseña incorrecta".
